...
func TestBinaryAndJSONFallback(t *testing.T) {
	for _, fallback := range []bool{false, true} {
		t.Run(map[bool]string{false: "binary", true: "json fallback"}[fallback], func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				w.Header().Set(vrpc.HeaderServer, "name=demo.server,version=1.0.0,instanceId=123e4567-e89b-12d3-a456-426614174000")
				if fallback {
				...
				encoded, _ := wire.Marshal(map[string]any{"result": map[string]any{"data": []byte{0, 255}}, "error": nil})
				_, _ = w.Write(encoded)
			}))
			defer server.Close()
			client, err := vrpc.NewClient(vrpc.Options{Endpoint: server.URL, Codec: cbor.Codec{}})
			if err != nil {
				t.Fatal(err)
			}
			params := struct {
			...
			}{Data: []byte{0, 255}, Map: map[int64][]byte{9007199254740993: {42}}}
			var result struct {
				Data []byte `json:"data" cbor:"data"`
			}
			_, err = client.Call(context.Background(), "demo.Service", "Get", params, &result)
...
func TestMalformedCBOR(t *testing.T) {
	...
	data, _ := wire.Marshal(map[string]any{"result": nil, "error": map[string]string{"code": "NOT_FOUND", "message": "missing"}})
